...
async function migrate() {
    ...
    const insertMany = newDb.transaction((rows) => {
        for (const row of rows) {
            try {
                insert.run(row.id, row.userId, row.projectId || null, row.sessionId, row.chatIndex, row.userQuery, row.userSummary || null, row.agentResponse, row.agentSummary || null, row.combo || null, row.isSummarized, row.priority, row.isPinned, row.referencedTasks, row.referencedKeypoints, row.referencedEntities, row.referencedProjects, row.linkedSessions, row.createdAt, row.lastAccessedAt);
                migrated++;
            }
            catch (err) {
    ...
    const entities = oldDb.prepare(`SELECT * FROM Entities`).all();
