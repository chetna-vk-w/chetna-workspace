...
export const hybridTools = [
    {
        ...
        handler: async (args) => {
            ...
            try {
                ...
                const graphParams = [pattern];
                if (userId) {
                    graphSql += ` AND userId = ?`;
                    graphParams.push(userId);
                }
                if (projectId) {
                    graphSql += ` AND projectId = ?`;
                    graphParams.push(projectId);
                }
                graphSql += ` ORDER BY createdAt DESC LIMIT ? OFFSET ?`;
                graphParams.push(limit, offset);
                const graphResults = db.prepare(graphSql).all(...graphParams).map((r) => ({
                    ...r,
