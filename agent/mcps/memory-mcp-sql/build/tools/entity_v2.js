    ...
    handler: async (args) => {
        ...
        try {
            switch (op) {
                case "create": {
                    if (!entityType || !name)
                        return { content: [{ type: "text", text: "entityType and name required" }], isError: true };
                ...
                case "search": {
                    ...
                    return { content: [{ type: "text", text: JSON.stringify({ count: entities.length, entities: entities.map((e) => ({ id: e.id, type: e.entityType, name: e.name })) }) }] };
                }
                default:
