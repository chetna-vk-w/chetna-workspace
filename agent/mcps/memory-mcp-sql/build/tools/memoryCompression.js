...
import { addShortTermChat, getShortTermChats, getSessionChatCount, summarizeAndMoveToLongTerm, searchShortTermMemory, searchLongTermMemory, getSessionSummaries, clearShortTermMemory, getOptimizedContext, updateMemoryPriority, pinMemory, getMemoryById, getLongTermMemoryStats, cleanupOldMemories, findCrossSessionMemories, db, createRelation, findDuplicateMemories, mergeDuplicateMemories, setMemoryTTL, removeMemoryTTL, searchMemoriesByDate, addMemoryTags, removeMemoryTags, searchMemoriesByTag, voteMemory, getMemoryVotes, bulkUpdatePriority, bulkAddTags, bulkDelete, bulkPin, updateMemoryQuality, getHighQualityMemories, calculateQualityScore, archiveMemory, unarchiveMemory, getArchivedMemories, createReminder, getPendingReminders, getUpcomingReminders, mergeMemories } from "../db/sqlite.js";
...
function detectIntent(userMsg) {
    ...
    if (['what', 'why', 'how', 'when', 'where', 'who', 'can', 'could', 'would', 'should', 'is', 'are', 'do', 'does'].includes(first))
        return 'question';
    if (ACTION_WORDS.has(first))
        return 'command';
    if (/error|fail|exception|bug/i.test(userMsg))
        return 'error';
    if (/success|done|completed|finished/i.test(userMsg))
        return 'success';
    if (/learn|realize|discover|understand/i.test(userMsg))
        return 'learning';
    if (/plan|will|should|might|consider/i.test(userMsg))
        return 'planning';
    return 'statement';
}
...
function smartTrim(context, maxChars) {
    ...
    for (const s of sentences) {
        if (result.length + s.length + 2 > maxChars)
            break;
        result += s + ". ";
    }
    ...
    if (result.length < maxChars * 0.5) {
        ...
        for (const w of words) {
            if (result.length + w.length + 1 > maxChars)
                break;
            result += w + " ";
        }
    }
...
function analyzeFlow(chats) {
    if (!chats.length)
        return { type: "empty", summary: "No conversation", complexity: 0 };
    ...
    let type = "general";
    if (errorCount > questionCount && errorCount > 0)
        type = "debugging";
    else if (questionCount > chats.length * 0.6)
        type = "learning";
    else if (successCount > 0 && type === "general")
        type = "progress";
...
function findRelatedByEntities(memory, allMemories) {
    const myEntities = new Set((memory.referencedEntities || []).concat(memory.referencedTasks || []).map((e) => e.toLowerCase()));
    ...
    allMemories.forEach(m => {
        if (m.id === memory.id)
            return;
        const theirEntities = new Set((m.referencedEntities || []).concat(m.referencedTasks || []).map((e) => e.toLowerCase()));
        let overlap = 0;
        myEntities.forEach(e => { if (theirEntities.has(e))
            overlap++; });
        if (overlap > 0)
            related.push({ id: m.id, overlap });
    });
    return related.sort((a, b) => b.overlap - a.overlap).slice(0, 3).map(r => r.id);
}
// Memory freshness score - recency + time decay
function freshnessScore(createdAt, lastAccessed, decayDays = 30) {
...
function suggestQueries(recentQueries) {
    const suggestions = [];
...
function weightedPriority(memory) {
...
function combo(user, agent, prog) {
...
function calculateRelevance(query, memory) {
    ...
    if (!qWords.length || !mWords.length)
        return 0;
    // Word overlap
        ...
        handler: async (args) => {
            try {
                ...
                if (entities.length > 0 && refs.tasks.length > 0) {
                    try {
                        const allEntities = db.prepare(`SELECT id, name FROM Entities WHERE userId = ?`).all(userId);
                        entities.forEach((entity) => {
                            const matched = allEntities.find((e) => e.name.toLowerCase() === entity.toLowerCase());
                            if (matched) {
                ...
                if (count > 0 && count % 10 === 0) {
                    try {
                        ...
                        if (dups.length > 0) {
                            dups.forEach((d) => mergeDuplicateMemories(d.original, d.duplicate));
        ...
        handler: async (args) => {
            try {
                ...
                if (preview) {
                    ...
                    const c = db.prepare(`SELECT COUNT(*) as c FROM LongTermMemory WHERE userId = ? AND createdAt < ? AND isPinned = 0`).get(userId, cutoff);
...
\`\`\``,
        ...
        handler: async (args) => {
            try {
                ...
                const insights = { questions: [], errors: [], progress: [], patterns: [] };
                recent.forEach((m) => {
...
\`\`\``,
        ...
        handler: async (args) => {
            try {
                ...
                summaries.slice(-3).forEach((s) => { context += `- ${s.summary}\n`; });
                context += "\n## Recent Messages\n";
        ...
        handler: async (args) => {
            try {
                ...
                if (sessionId) {
                    ...
                    const allChats = db.prepare(`SELECT * FROM ShortTermChat WHERE userId = ? AND createdAt > ? ORDER BY createdAt DESC LIMIT 100`).all(userId, cutoff);
                    chats = allChats;
                }
                if (chats.length === 0) {
                ...
                const entities = chats.flatMap(c => {
                    try {
                        return JSON.parse(c.referencedEntities || "[]");
                    }
                    catch {
                        return [];
                    }
...
\`\`\``,
        ...
        handler: async (args) => {
            try {
                ...
                const longTerm = db.prepare(`SELECT * FROM LongTermMemory WHERE userId = ? ORDER BY createdAt DESC LIMIT ?`).all(userId, limit);
                const summaries = db.prepare(`SELECT * FROM SessionSummary WHERE userId = ? ORDER BY createdAt DESC LIMIT 20`).all(userId);
                let shortTerm = [];
                if (includeShortTerm) {
                    shortTerm = db.prepare(`SELECT * FROM ShortTermChat WHERE userId = ? ORDER BY createdAt DESC LIMIT ?`).all(userId, limit);
                }
                const exportData = {
                    ...
                    longTermMemories: longTerm.map(m => ({
                        ...
                        response: m.responseSummary,
                        keywords: (() => { try {
                            return JSON.parse(m.keywords || "[]");
                        }
                        catch {
                            return [];
                        } })(),
                        entities: (() => { try {
                            return JSON.parse(m.entities || "[]");
                        }
                        catch {
                            return [];
                        } })(),
                    ...
                    for (const m of data.longTermMemories) {
                        try {
                            ...
                            db.prepare(`INSERT INTO LongTermMemory 
                                ...
                                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
                                .run(id, userId, projectId || null, m.query, m.summary, m.response || '', m.summary || '', '', JSON.stringify(m.keywords || []), JSON.stringify(m.entities || []), m.priority || 0.5, m.pinned ? 1 : 0, m.createdAt || new Date().toISOString());
                            imported.longTerm++;
                        }
...
\`\`\``,
        ...
        handler: async (args) => {
            try {
                ...
                const m1 = db.prepare(`SELECT linkedSessions FROM LongTermMemory WHERE id = ?`).get(memoryId1);
                const m2 = db.prepare(`SELECT linkedSessions FROM LongTermMemory WHERE id = ?`).get(memoryId2);
                if (!m1 && !m2)
                    return { isError: true, content: [{ type: "text", text: "One or both memories not found" }] };
                if (m1) {
    ...
    // 21. fuzzy_recall - fuzzy search with typo tolerance
    {
        ...
        handler: async (args) => {
            try {
                ...
                const all = db.prepare(`SELECT id, content, summary, response FROM LongTermMemory WHERE userId = ? ORDER BY createdAt DESC LIMIT 100`).all(userId);
                ...
                return { content: [{ type: "text", text: JSON.stringify({
                                ...
                                memories: fuzzyResults.slice(offset, offset + limit).map((r) => ({
                                    id: r.id,
                                    query: r.content?.slice(0, 60),
        ...
        handler: async (args) => {
            try {
                ...
                switch (operation) {
                    ...
                    case "add_tags":
                        if (!tags?.length)
                            return { isError: true, content: [{ type: "text", text: "Tags required for add_tags" }] };
    ...
    // 30. merge - merge two memories
    {
        ...
        handler: async (args) => {
            try {
                ...
                if (!result.success)
                    return { isError: true, content: [{ type: "text", text: result.error || "Merge failed" }] };
