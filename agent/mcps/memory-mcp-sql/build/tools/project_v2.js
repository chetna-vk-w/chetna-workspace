...
\`\`\``,
    ...
    handler: async (args) => {
        ...
        try {
            ...
            if (op === "list_projects") {
                const projects = db.prepare(`SELECT id, name, description FROM Projects WHERE userId = ?`).all(userId);
            ...
            if (op === "plan_task") {
                if (!projectId || !title)
                    return { content: [{ type: "text", text: "projectId and title required" }], isError: true };
            ...
            if (op === "list_tasks") {
                const tasks = db.prepare(`SELECT id, title, status FROM Tasks WHERE projectId = ?`).all(projectId);
            ...
            if (op === "plan_workflow") {
                if (!projectId || !name || !steps)
                    return { content: [{ type: "text", text: "projectId, name, steps required" }], isError: true };
            ...
            if (op === "get_workflow") {
                if (!id)
                    return { content: [{ type: "text", text: "id required" }], isError: true };
                const w = db.prepare(`SELECT * FROM Workflows WHERE id = ?`).get(id);
                if (!w)
                    return { content: [{ type: "text", text: "Not found" }] };
                return { content: [{ type: "text", text: JSON.stringify({ id: w.id, name: w.name, status: w.status }) }] };
            }
            if (op === "list_workflows") {
                const workflows = db.prepare(`SELECT id, name, status FROM Workflows WHERE projectId = ?`).all(projectId);
