        ...
        try {
            switch (op) {
                case "better": {
                    ...
                    const memories = db.prepare(`SELECT * FROM Timeline WHERE ownerId = ? AND createdAt > ? ORDER BY createdAt DESC LIMIT ?`).all(userId, cutoff, limit ?? 50);
                    const timeline = {};
                    memories.forEach((m) => {
                    ...
                    let relations = {};
                    if (includeRelations) {
                        const entities = db.prepare(`SELECT * FROM Entities WHERE ownerId = ? ORDER BY createdAt DESC LIMIT 20`).all(userId);
                        relations = entities.reduce((acc, e) => { acc[e.name] = { id: e.id, type: e.entityType }; return acc; }, {});
                    }
                ...
                case "chat_get": {
                    const chats = db.prepare(`SELECT * FROM ChatHistory WHERE userId = ? ORDER BY createdAt DESC LIMIT ?`).all(userId, limit ?? 20);
                    return { content: [{ type: "text", text: JSON.stringify({ chats: chats.length, recent: chats.map((c) => ({ role: c.role, content: c.content?.slice(0, 100) })) }) }] };
                }
                case "chat_summary": {
                    if (!summary)
                ...
                case "get_summary": {
                    const summaries = db.prepare(`SELECT * FROM SessionSummary WHERE userId = ? ORDER BY createdAt DESC LIMIT ?`).all(userId, limit ?? 5);
                    return { content: [{ type: "text", text: JSON.stringify({ summaries: summaries.map((s) => ({ id: s.id, summary: s.summary, createdAt: s.createdAt })) }) }] };
                }
                default:
