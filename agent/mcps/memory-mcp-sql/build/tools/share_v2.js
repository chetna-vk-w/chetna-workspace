...
\`\`\``,
    ...
    handler: async (args) => {
        ...
        try {
            switch (op) {
                case "share": {
                    if (!toOwnerId || !content)
                        return { content: [{ type: "text", text: "toOwnerId and content required" }], isError: true };
                ...
                case "shared_with_me": {
                    const shared = db.prepare(`SELECT * FROM SharedMemories WHERE toOwnerId = ? ORDER BY createdAt DESC LIMIT 20`).all(userId);
                    return { content: [{ type: "text", text: JSON.stringify({ received: shared.length, items: shared.map((s) => ({ from: s.fromOwnerId, content: s.content?.slice(0, 100), read: s.isRead })) }) }] };
                }
                case "shared_by_me": {
                    const shared = db.prepare(`SELECT * FROM SharedMemories WHERE fromOwnerId = ? ORDER BY createdAt DESC LIMIT 20`).all(userId);
                    return { content: [{ type: "text", text: JSON.stringify({ sent: shared.length, items: shared.map((s) => ({ to: s.toOwnerId, content: s.content?.slice(0, 100) })) }) }] };
                }
                case "get_network": {
                    if (!personId)
                        return { content: [{ type: "text", text: "personId required" }], isError: true };
                    const entity = db.prepare(`SELECT * FROM Entities WHERE userId = ? AND name LIKE ? LIMIT 1`).get(userId, `%${personId}%`);
                    if (!entity)
                        return { content: [{ type: "text", text: "Person not found" }] };
                    const relations = db.prepare(`SELECT * FROM Relations WHERE fromId = ? OR toId = ?`).all(entity.id, entity.id);
                    return { content: [{ type: "text", text: JSON.stringify({ person: entity.name, relations: relations.length }) }] };
                }
                case "person_memories": {
                    if (!personId)
                        return { content: [{ type: "text", text: "personId required" }], isError: true };
                    const memories = db.prepare(`SELECT * FROM Timeline WHERE ownerId = ? AND entities LIKE ? ORDER BY createdAt DESC LIMIT 20`).all(userId, `%${personId}%`);
