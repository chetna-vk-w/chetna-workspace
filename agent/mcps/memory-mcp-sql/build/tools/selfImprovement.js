...
import { createReflection, getUserReflections, analyzeMistakePatterns, getAverageRating, createImprovementSuggestion, getUserSuggestions, updateSuggestionStatus, createWorkingBuffer, getWorkingBuffer, updateWorkingBuffer, completeWorkingBuffer, abortWorkingBuffer, getUserWorkingBuffers } from "../db/sqlite.js";
...
export const selfImprovementTools = [
    {
        ...
        handler: async (args) => {
            try {
                ...
                const reflection = createReflection(userId, projectId || null, taskId || null, evaluation, mistakes, learnings, evaluation.overallRating || 3);
    ...
    },
    {
        ...
        handler: async (args) => {
            try {
                ...
                const reflection = createReflection(userId, projectId || null, taskId || null, { errorDescription, severity, resolved: !!resolution }, [{ description: errorDescription, severity, resolved: !!resolution }], lessons, severity === "critical" ? 1 : severity === "high" ? 2 : 3);
                if (lessons.length > 0) {
                    lessons.forEach((lesson) => {
                        createImprovementSuggestion(userId, projectId || null, `From error "${errorDescription.slice(0, 50)}": ${lesson}`, reflection.id);
                    });
                }
                return {
    ...
    },
    {
        ...
        handler: async (args) => {
            try {
                ...
                const buffer = createWorkingBuffer(userId, projectId || null, operationType, totalSteps, initialState);
                return {
